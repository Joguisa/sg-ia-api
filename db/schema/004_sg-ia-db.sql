/***********************************************************************************
 * Nombre      : sg_ia_unified_schema.sql
 * Descripción : Esquema unificado completo de la base de datos SG-IA
 *               Sistema de salas, multiidioma, roles, batches, logs y vistas analíticas
...
CREATE TABLE system_prompts (
    ...
    default_language       ENUM('es', 'en') NOT NULL DEFAULT 'es' COMMENT 'Idioma por defecto para generación',
...
-- =========================================================
-- 2. GESTIÓN DE CONTENIDO (Batches)
-- =========================================================
...
-- =========================================================
-- 3. ENTIDADES DE JUEGO (Players, Rooms, Questions)
-- =========================================================
...
CREATE TABLE game_rooms (
    ...
    language            ENUM('es', 'en') NOT NULL DEFAULT 'es' COMMENT 'Idioma de la interfaz',
...
-- =========================================================
-- 4. TABLAS TRANSACCIONALES (Sessions, Answers, Logs)
-- =========================================================
...
FOR EACH ROW
BEGIN
    IF NEW.is_correct = 1 AND EXISTS (
        SELECT 1 FROM question_options 
        WHERE question_id = NEW.question_id AND is_correct = 1
    ) THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Solo una opción correcta por pregunta';
    END IF;
END$$
...
FOR EACH ROW
BEGIN
    IF NEW.is_correct = 1 AND OLD.is_correct <> 1 AND EXISTS (
        SELECT 1 FROM question_options 
        WHERE question_id = NEW.question_id AND is_correct = 1
    ) THEN
        SIGNAL SQLSTATE '45000' 
...
-- 6.2 Estadísticas generales de sala
...
-- 6.3 Estadísticas de jugadores por sala
...
-- 6.4 Estadísticas de preguntas por sala
...
SELECT
    ...
    SUM(1 - pa.is_correct) AS incorrect_count,
    COALESCE(ROUND(SUM(1 - pa.is_correct) / NULLIF(COUNT(pa.id), 0) * 100, 2), 0) AS error_rate,
    COALESCE(ROUND(AVG(pa.time_taken_seconds), 2), 0) AS avg_time_sec
FROM questions q
...
-- 6.5 Estadísticas por categoría por sala
...
-- 6.6 Estadísticas de jugador por categoría (global)
...
-- Admin inicial (superadmin) - Password: Admin123!
INSERT INTO admins (email, password_hash, role, is_active) VALUES
    ('admin@sg-ia.com', '$2y$12$te4rWeMY9jRnwrqK2wvqd.ZepWvkXunhOLQu5gRA5tfG/R4Ck3Weq', 'superadmin', 1);
...
SELECT 
    'Esquema SG-IA creado exitosamente' AS mensaje,
    (SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'sg_ia_db' AND TABLE_TYPE = 'BASE TABLE') AS tablas,
    (SELECT COUNT(*) FROM information_schema.VIEWS WHERE TABLE_SCHEMA = 'sg_ia_db') AS vistas,
    (SELECT COUNT(*) FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA = 'sg_ia_db') AS triggers;