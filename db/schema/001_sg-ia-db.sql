...
-- =========================================================
-- 3. TABLAS TRANSACCIONALES (Sesiones y Logs)
-- =========================================================
...
SELECT pa.session_id, COUNT(*) AS questions_answered, AVG(pa.is_correct) AS accuracy, AVG(pa.time_taken_seconds) AS avg_time_sec
FROM player_answers pa GROUP BY pa.session_id;

CREATE VIEW v_player_topic_stats AS
SELECT p.id AS player_id, qc.id AS category_id, qc.name AS category_name, COUNT(pa.id) AS answers, AVG(pa.is_correct) AS accuracy, AVG(pa.time_taken_seconds) AS avg_time_sec
FROM players p
...
SELECT gs.id AS session_id, gs.player_id, gs.status, gs.started_at, gs.ended_at,
...
FROM game_sessions gs LEFT JOIN player_answers pa ON pa.session_id = gs.id
