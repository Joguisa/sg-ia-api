/***********************************************************************************
Nombre           : sg_ia_db_v3_final.sql
Descripción      : Script unificado para la BDD del Serious Game de Alfabetización Sanitaria. 
                   Incluye: Tablas core, Sistema de Salas, Multi-idioma y Roles de Admin.
Proyecto         : Serious Game con IA — Alfabetización Sanitaria Cáncer de Colon
Autor            : Jonatán Guillén
Fecha de actualización: 08/01/2026 (v3.1)
***********************************************************************************/
...
-- Tabla Admins unificada con columna 'role' y eliminado lógico
CREATE TABLE admins (
  id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
-- Tabla System Prompts unificada con 'default_language'
...
-- Tabla Questions unificada con 'language'
...
-- =========================================================
-- 6. ANALÍTICA (Vistas unificadas)
-- =========================================================

CREATE VIEW v_batch_statistics AS
SELECT
  qb.id, qb.batch_name, qb.batch_type, qb.language, qb.total_questions, qb.verified_count,
  ROUND((qb.verified_count / NULLIF(qb.total_questions, 0) * 100), 2) AS verification_percent,
  qb.imported_at, qb.status
FROM question_batches qb;

CREATE VIEW v_room_statistics AS
SELECT
  gr.id AS room_id, gr.room_code, gr.name AS room_name, gr.status AS room_status,
...
('Tamizaje y Detección', 'Métodos de diagnóstico.'),
('Prevención y Estilos de Vida', 'Hábitos saludables.');

-- Admin inicial como Superadmin
INSERT INTO admins (email, password_hash, role) 
VALUES ('admin@sg-ia.com', '$2y$12$te4rWeMY9jRnwrqK2wvqd.ZepWvkXunhOLQu5gRA5tfG/R4Ck3Weq', 'superadmin');
...
VALUES ('Eres un experto oncólogo... [JSON Valid Strictly]', 0.7, 'auto', 15, 'es', 1);