...
// CAMBIADO: GroqAIService -> DeepSeekAIService
final class DeepSeekAIService implements GenerativeAIInterface
{
    ...
    public function generateQuestion(string $topic, int $difficulty): array
    {
        ...
        try {
            ...
            if (!isset($body['choices'][0]['message']['content'])) {
                error_log("Unexpected DeepSeek response: " . json_encode($body));
                throw new \RuntimeException('Respuesta inesperada de DeepSeek API');
            }

            $deepseekText = $body['choices'][0]['message']['content'];
            return $this->parseAIResponse($deepseekText);
        } catch (GuzzleException $e) {
            if ($this->isRateLimitError($e)) {
                throw new \RuntimeException('RATE_LIMIT_EXCEEDED: ' . $e->getMessage());
            }
            throw new \RuntimeException('Error en solicitud a DeepSeek API: ' . $e->getMessage());
        } catch (\Throwable $e) {
            throw new \RuntimeException('Error procesando respuesta de DeepSeek: ' . $e->getMessage());
        }
    }
    ...
    // AÑADE ESTOS MÉTODOS FALTANTES (copiados de GeminiAIService y adaptados)
    ...
    private function parseAIResponse(string $aiText): array
    {
        ...
        return [
            ...
            'explanation_correct' => trim($json['explanation_correct'] ?? 'Sin explicación'),
            'explanation_incorrect' => trim($json['explanation_incorrect'] ?? 'Sin explicación'),
